...
def find_prosite_patterns(dict_sequences, regex_pattern):
    matches = []
...
def find_prosite_patterns_in_db(database, pattern):
    cursor = database.cursor()

    query = '''
            SELECT DISTINCT header_id, REGEXP_SUBSTR(sequence, %s) AS matched_sequence
            FROM Sequences
            WHERE sequence REGEXP %s
        '''

    # Parameterized Query
    # (pattern,) is a tuple containing only one element (...,): the prosite pattern as regex
    ...
    for row in cursor.fetchall():
        header_id, matched_sequence = row  # Unpack the tuple
        matches.append({'header_id': header_id, 'matched_sequence': matched_sequence})
...
def main():
    ...
    pattern = prosite_to_regex(prosite_pattern)     # still a string here
    ...
    if args.fasta:
        ...
        matches = find_prosite_patterns(dict_sequences, regex_pattern)
        ...
        try:
            matches = find_prosite_patterns_in_db(connection, pattern)
            for match in matches:
                print(f'{match["header_id"]}\t{match["matched_sequence"]}')
