...
def parse_pdb(atom_type):
    """Read and parse PDB data from stdin, returning a list of relevant atoms."""
    atoms = []
    model_started = False
    for line in sys.stdin:
        if line.startswith('MODEL'):
            if model_started:  # If the first model has already started, we stop reading further.
                break
            else:
                model_started = True  # Mark that the first model has started.
        elif line.startswith('ENDMDL'):
            break  # End reading after the first model.
        elif line.startswith('ATOM') and line[12:16].strip() == atom_type:
            chain = line[21]
